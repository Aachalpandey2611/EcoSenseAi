    ...
    >
      <GlassPanel>
        ...
        <div className="mb-6 text-center">
          ...
          <p className="text-slate-400 text-sm">Sign in to EcoSense AI</p>
        </div>
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-semibold transition-all duration-300 ${
                activeTab === tab.id
                  ? tab.id === 'admin'
                    ...
                    : 'bg-brand-500/20 text-brand-300 shadow-lg border border-brand-500/30'
                  : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              <span className={activeTab === tab.id ? (tab.id === 'admin' ? 'text-purple-400' : 'text-brand-400') : ''}>
                {tab.icon}
              </span>
              {tab.label}
            </button>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'demo' && (
            ...
            >
              {/* Info box */}
              <div className="mb-5 p-4 rounded-xl bg-brand-500/8 border border-brand-500/20">
                <p className="text-xs text-brand-300 font-medium mb-1">🌱 Demo Account</p>
                <p className="text-xs text-slate-400">Explore EcoSense AI with a pre-filled demo account. No sign-up needed.</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-end">
                    <Link to="/auth/forgot-password" className="text-sm font-medium text-brand-400 hover:text-brand-300 transition-colors">
                      Forgot password?
                    </Link>
                  </div>
                </div>
                ...
                <Button
                  type="button"
                  variant="secondary"
                  className="w-full border-brand-500/40 hover:bg-brand-500/10 text-brand-400"
          ...
          {activeTab === 'admin' && (
            ...
            >
              ...
              <div className="mb-5 p-4 rounded-xl bg-purple-500/8 border border-purple-500/20">
                ...
                <p className="text-xs text-slate-400">Access restricted to <span className="text-purple-300 font-medium">admin</span> and <span className="text-purple-300 font-medium">super_admin</span> roles. You will be redirected to the admin dashboard.</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full flex items-center justify-center gap-2 py-3 px-6 rounded-xl font-semibold text-white transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
