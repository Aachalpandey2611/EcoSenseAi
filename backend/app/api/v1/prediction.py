...
from app.api.deps import get_current_active_user, require_pro_tier
...
@router.post("/train")
async def train_models(current_user: User = Depends(require_pro_tier)):
...
@router.get("/forecast")
async def get_forecast(days: int = 30, current_user: User = Depends(require_pro_tier)):
